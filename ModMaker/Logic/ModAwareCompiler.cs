...
namespace ModMaker
{
    ...
    public class Compiler
    {
        ...
        public void RunCommand(string Command, string FilePath, bool LogWindow = false)
        {
            ...
            {
                //if we cannot find Steam we're in trouble
                if (Buffer.ToString().Contains("{SteamPath}")) throw;
            }
            
            Buffer.Replace("{FilePath}", KeyValues.Quote(FilePath));
            Buffer.Replace("{ModMakerPath}", SourceFileSystem.FormatFolderPath(Application.StartupPath));
            ...
            if (LogWindow)
            {
                ConsoleProcessMonitorForm Log = new ConsoleProcessMonitorForm();
            ...
            else
            {
                try
                {
                    Process.Start(Command.Substring(0, Command.IndexOf(" ")), Command.Substring(Command.IndexOf(" ")));
                }
                catch (Exception)
                {
